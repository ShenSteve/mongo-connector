...
class DocManager(DocManagerBase):
    ...
    def _index_and_mapping(self, namespace):
        ...
        db, tb = namespace.split(".", 1)
        doc_type = '_doc'
        index = '{db}_{tb}'.format(db=db.lower(), tb=tb)
    ...
    def handle_command(self, doc, namespace, timestamp):
        ...
        if doc.get("create"):
            ...
            if db and coll:
                ...
                warnings.warn(
                    "by pass table creation in mongodb, in ES7' index creation is as lazy module. %s on index %s." % (coll, db)
                )
